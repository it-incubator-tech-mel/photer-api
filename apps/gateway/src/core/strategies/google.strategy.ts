...
export class GoogleStrategy extends PassportStrategy(Strategy, 'google') {
  ...
  async validate(
    ...
    if (!foundUser) {
      ...
      user = await this.authService.createUserWhenRegistrationByProvider(usernameFromProvider, email)
      ...
      if (!oauthAccount) {
        let findCreatedUser = await this.userRepository.findByUsername(user.getUsername());
        await this.authService.createOauthBodyAndAddToDB(findCreatedUser.getId(),ProviderType.GOOGLE, id, email)
